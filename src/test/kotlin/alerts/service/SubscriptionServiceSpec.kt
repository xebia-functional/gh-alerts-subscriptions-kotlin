...
class SubscriptionServiceSpec(
    ...
    "If repo exists, and repo has no subscribers then event is send to Kafka" {
        ...
        kafka.subscriptionConsumer().receiveAutoAck().asFlow().map {
            it.key() shouldBe SubscriptionKey(subscription.repository)
            it.value() shouldBe SubscriptionEventRecord(SubscriptionEvent.Created)
        }
    }
...
private fun Kafka.subscriptionConsumer(): ReactiveKafkaConsumerTemplate<SubscriptionKey, SubscriptionEventRecord> =
